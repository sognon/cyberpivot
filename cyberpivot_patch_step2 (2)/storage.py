    ...
    with get_conn() as c:
        c.executescript("""
            ...
            CREATE TABLE IF NOT EXISTS responses (
                ...
                evidence_json TEXT,
                updated_at TEXT NOT NULL,
                UNIQUE(audit_id, qid, item),
                FOREIGN KEY(audit_id) REFERENCES audits(id)
            );
        """)

# Ensure unique index for (audit_id, qid, item) for existing DBs
def ensure_indexes():
    with get_conn() as c:
        c.execute("CREATE UNIQUE INDEX IF NOT EXISTS uniq_responses_aqi ON responses(audit_id, qid, item)")
